          ...
          const aggregated =
            seasons.length === 0 || !displaySeason
              ...
              : {
                  ...
                  appearances: seasons.reduce((sum, season) => sum + season.stats.appearances, 0),
                  goals: seasons.reduce((sum, season) => sum + season.stats.goals, 0),
                  assists: seasons.reduce((sum, season) => sum + season.stats.assists, 0),
                }

          const badgeBg = aggregated ? team?.primary_color : undefined
          const badgeStyle = badgeBg ? { background: badgeBg, color: contrastText(badgeBg) } : undefined
              ...
              <div className="min-w-0">
                {aggregated ? (
                  <div className="flex min-w-0 items-center gap-2 text-sm">
                    {fromTeam && (
                      <>
                        ...
                        <span
                          ...
                          title={gameT(hadLoan ? 'season.loan' : 'timeline.changeClub')}
                        >
                          →
                        </span>
                      </>
                    ...
                    {hadLoan && !fromTeam && (
                      <span className="text-white/45" title={gameT('season.loan')}>↳</span>
                    )}
                  ...
                  <span className="pulse-pick text-sm text-white/70">{gameT('season.choosingClub')}</span>
                ) : isCurrent && decidingCareer ? (
                  <span className="pulse-pick text-sm text-white/70">{gameT('season.careerDecision')}</span>
                ) : (
                ...
                {aggregated && (
                  <div className="mt-1 flex min-w-0 flex-wrap items-center gap-1">
                    {competition && (
                      ...
                      >
                        {competition.logo_url && (
                          <img src={competition.logo_url} alt="" className="h-3 w-3 shrink-0 object-contain" />
                        )}
                    ...
                    {hadLoan && <TimelineTag>{gameT('season.loan')}</TimelineTag>}
                    {hadPromotion && <TimelineTag tone="positive">{gameT('season.promotion')}</TimelineTag>}
                    {hadRelegation && <TimelineTag tone="negative">{gameT('season.relegation')}</TimelineTag>}
                    {hadInjury && <TimelineTag tone="warning">{gameT('season.injury')}</TimelineTag>}
                    {hadSuspension && <TimelineTag tone="warning">{gameT('season.suspended')}</TimelineTag>}
                  </div>
                )}
              </div>
              <div>
      ...
      <div className="flex flex-col gap-2 border-t border-sky-400/25 bg-gradient-to-r from-sky-500/15 to-black/40 px-4 py-3">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            <div>
              <div className="text-[10px] font-bold uppercase tracking-wide text-sky-200/90">{gameT('national.title')}</div>
              <span className="font-semibold text-white">
                {countryDisplayName(locale, country) || gameT('national.title')}
              </span>
            </div>
        ...
        {(() => {
          ...
          if (!last) return <div className="text-[11px] font-medium text-white/55">{gameT('national.none')}</div>
          return (
...
function TimelineTag({
  ...
  const classes = {
    neutral: 'bg-sky-500/25 text-sky-100',
    positive: 'bg-emerald-500/25 text-emerald-100',
    negative: 'bg-rose-500/25 text-rose-200',
    warning: 'bg-amber-500/25 text-amber-100',
  }[tone]
  return (
    <span className={`rounded-md px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide ${classes}`}>
      {children}
    </span>
  )
}
